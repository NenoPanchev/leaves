...
public class PredicateBuilderV1<ENTITY> {
    private final Root<ENTITY> root;
    private final CriteriaBuilder builder;
    private final List<Predicate> predicates = new ArrayList<>();

    public PredicateBuilderV1(Root<ENTITY> root, CriteriaBuilder builder) {
        this.root = root;
        this.builder = builder;
    }

    public <T> PredicateBuilderV1<?> in(final SingularAttribute<?, T> attribute, final Collection<T> list) {
    ...
    public PredicateBuilderV1<?> inIgnoreCase(final SingularAttribute<?, String> attribute, final Collection<String> list) {
        if (list != null && !list.isEmpty()) {
    ...
    public <T> PredicateBuilderV1<ENTITY> joinLike(final SingularAttribute<?, T> attribute,
                                                   final String value,
    ...
    public <T, D> PredicateBuilderV1<ENTITY> joinIn(final ListAttribute<?, T> attribute,
    ...
    public <T, D> PredicateBuilderV1<ENTITY> joinInLike(final ListAttribute<?, T> attribute,
                                                        final Collection<D> list,
                                                        final String fieldName) {
        if (list != null && !list.isEmpty()) {
            final ListJoin<ENTITY, T> roleJoin = root.join((ListAttribute<? super ENTITY, T>) attribute);
...
// todo            this.predicates.add(roleJoin.get(fieldName).in(list));
    ...
    public <T extends Number> PredicateBuilderV1<?> compare(final SingularAttribute<?, T> attribute,
    ...
    public PredicateBuilderV1<?> like(final SingularAttribute<?, String> attribute,
                                      final String value) {
    ...
    public PredicateBuilderV1<?> equalsIgnoreCase(final SingularAttribute<?, String> attribute,
                                                  final String value) {
        if (value != null) {
    ...
    public <T> PredicateBuilderV1<?> equals(final SingularAttribute<?, T> attribute,
                                            final T value) {
        if (value != null) {
